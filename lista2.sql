...
-- (x)
select cl.Nome_RazaoSocial, st.NomeSoftware, count(li.NumLicenca) as TOTAL from llicenca li
INNER JOIN lcliente as cl on cl.idCLIENTE = li.idCLIENTE_FK
INNER JOIN lsoftware  as st on li.idSOFTWARE_FK_FK = st.idSOFTWARE
group by cl.Nome_RazaoSocial, st.idSOFTWARE order by cl.Nome_RazaoSocial asc;

-- (y)
select cl.Nome_RazaoSocial as NOME, count(li.NumLicenca) as TOTAL from lcliente as cl
INNER JOIN llicenca as li ON cl.idCliente = li.idCliente_FK
INNER JOIN ltipo_empresa as em ON cl.idTIPO_EMPRESA_FK = em.idTIPO_EMPRESA
group by li.idCLIENTE_FK having count(l.NumLicenca) > 10;
